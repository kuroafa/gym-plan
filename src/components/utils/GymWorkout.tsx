...
const GymWorkout = (props: Props) => {
  ...
  useEffect(() => {
    if (currentWorkoutIndex < GymBuildingWorkouts.length) {
      const currentWorkout = GymBuildingWorkouts[currentWorkoutIndex];
      if (!isResting) {
  ...
  return (
    <div>
      ...
      <button  className=" rounded-full bg-gray-300 font-medium text-xl whitespace-nowrap"   onClick={handleOpen}><Image width={30} height={30} alt="play" src='/play-button.png'/></button>
    </div>
  );
};
